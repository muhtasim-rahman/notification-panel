...
let currentTab = "all"; // Tracks the active tab
...
function renderTabs() {
  ...
  const tabsHTML = `
    ...
    <ul>
      <li title="All">
        <label for="tab1" role="button"><span>All</span></label>
      </li>
      <li title="Unread">
        <label for="tab2" role="button"> <span>Unread</span> </label>
      </li>
      <li title="Pinned">
        <label for="tab3" role="button"><span>Pinned</span></label>
      </li>
    </ul>
  ...
  tabsContainer.querySelector("#tab1").addEventListener("change", () => {
    currentTab = "all";
    displayNotifications(currentPage);
  });

  tabsContainer.querySelector("#tab2").addEventListener("change", () => {
    currentTab = "unread";
    displayNotifications(currentPage);
  });

  tabsContainer.querySelector("#tab3").addEventListener("change", () => {
    currentTab = "pinned";
    displayNotifications(currentPage);
  });
...
function displayNotifications(page) {
  ...
  renderNotificationHeader(); // Add header
  renderTabs(); // Add tabs before notifications

  const startIndex = (page - 1) * itemsPerPage;
  let filteredNotifications = activeNotifications;

  if (currentTab === "unread") {
    filteredNotifications = activeNotifications.filter(
      (notification) =>
    ...
    filteredNotifications = activeNotifications.filter(
      (notification) =>
        localStorage.getItem(`pinned-${notification.id}`) === "true" ||
        notification.pined === "Yes"
    );
  }

  if (filteredNotifications.length === 0) {
    const noNotificationImg =
      currentTab === "pinned"
        ? "https://via.placeholder.com/400x300.png?text=No+Pinned+Notifications" // Change this to your desired image URL
        : "https://i.ibb.co.com/YpvK8h7/no-notifications-img.jpg";

    panel.innerHTML += `<img class="no_notifications_img" src="${noNotificationImg}" alt="No notifications available right now!" />`;
    return;
  }
    ...
    notificationDiv.innerHTML = `
      ...
      <div class="menu">
        ...
        <div class="menu-options">
          ...
          <button onclick="togglePinStatus('${notification.id}', this)">
            ${isPinned ? "Unpin" : "Pin"}
          </button>
  ...
  renderPagination(filteredNotifications.length); // Add pagination after notifications
...
function togglePinStatus(id, button) {
  ...
  if (isPinned) {
    ...
    localStorage.setItem(`pinned-${id}`, "true");
